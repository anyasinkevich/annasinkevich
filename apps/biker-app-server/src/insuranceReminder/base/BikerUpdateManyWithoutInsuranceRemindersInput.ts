/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { BikerWhereUniqueInput } from "../../biker/base/BikerWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class BikerUpdateManyWithoutInsuranceRemindersInput {
  @Field(() => [BikerWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [BikerWhereUniqueInput],
  })
  connect?: Array<BikerWhereUniqueInput>;

  @Field(() => [BikerWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [BikerWhereUniqueInput],
  })
  disconnect?: Array<BikerWhereUniqueInput>;

  @Field(() => [BikerWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [BikerWhereUniqueInput],
  })
  set?: Array<BikerWhereUniqueInput>;
}

export { BikerUpdateManyWithoutInsuranceRemindersInput as BikerUpdateManyWithoutInsuranceRemindersInput };
